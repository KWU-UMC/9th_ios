                ...
                VStack{
                    VStack(spacing: 40){ //Part2.1 - id,pw
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            TextField("아이디", text: $viewModel.id).font(.PretendardMedium16).foregroundColor(.gray03)
                            Rectangle().frame(height: 1).foregroundColor(.gray02)
                        }
                        
                        //pw
                        VStack(alignment: .leading, spacing: 6) {
                            SecureField("비밀번호", text: $viewModel.pwd).font(.PretendardMedium16).foregroundColor(.gray03)
                            Rectangle().frame(height: 1).foregroundColor(.gray02)
                        }
                    }
                    Spacer(minLength: 75)
                    ...
                    VStack{ //2.2 - Login Button & 회원가입
                        ...
                        .shadow(color: .black.opacity(0.25), radius: 12, x: 0, y: 8)
                        // 버튼을 누르는 '동작'은 .simultaneousGesture로 실행
